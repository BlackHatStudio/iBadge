...
$npmCmd = Get-NpmCmd
Invoke-Process -FilePath $npmCmd -ArgumentList @('run', 'build') -WorkingDirectory $RepoRoot
& (Join-Path $scriptRoot 'stage-webapp-package.ps1') -RepoRoot $RepoRoot -StageRoot $StageRoot

$resolvedStageRoot = (Resolve-Path $StageRoot).Path
$resolvedIsccPath = Get-IsccPath -PreferredPath $IsccPath
$issPath = Join-Path $RepoRoot 'deployment\installer\iBadgeWebAppInstaller.iss'
